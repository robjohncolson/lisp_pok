...
class POKEngine:
    ...
    def _load_curriculum(self, file_path: str) -> List[Question]:
        try:
            with open(file_path, 'r') as f:
                data = json.load(f)
                return [Question(id=q.get('id'), prompt=q.get('prompt'), qtype=q.get('type', 'mcq'),
                                 choices=q.get('attachments', {}).get('choices', []),
                                 answer_key=q.get('attachments', {}).get('answerKey')) for q in data]
    ...
    def add_node(self, pubkey: str, archetype: str, provisional_reputation: Optional[float] = None) -> Node:
        ...
        node = Node(pubkey=pubkey, archetype=archetype, reputation=provisional_reputation)
    ...
    def create_txn(self, qid: str, pubkey: str, ans: str, t: float, txn_type: str) -> Transaction:
        ...
        return Transaction(id=f"{t}-{pubkey[:5]}-{txn_type}", timestamp=t, owner_pubkey=pubkey,
                           question_id=qid, type=txn_type, payload=Payload(answer=str(ans), hash=txn_hash))
    ...
    def calculate_convergence(self, node: Node, qid: str, weighted: bool = False) -> float:
        ...
        for txn in attns:
            ...
            if attester_pubkey not in self.nodes: continue
    ...
    def propose_pok_block(self, node: Node):
        ...
        for txn in node.mempool:
            if txn.type == "completion" and txn.owner_pubkey == node.pubkey:
                ...
                attns_for_txn = [t for t in all_visible_txns if t.question_id == txn.question_id and t.type == "attestation"]
                
                if len(attns_for_txn) >= min_attest and self.calculate_convergence(node, txn.question_id) >= self.quorum_conv_thresh:
                    minable_completions.append(txn)

        if not minable_completions: return
